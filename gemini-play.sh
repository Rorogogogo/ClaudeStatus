#!/bin/bash
# Notchy Antigravity (Gemini CLI) state-update hook.
# Invoked by Antigravity lifecycle hooks. Reads hook payload JSON from stdin
# when available and writes the current status to ~/.gemini/notchy/status.
#
# Usage: gemini-play.sh <event-arg>
# Event args:
